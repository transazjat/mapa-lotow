  ...
  return new Promise(
    (resolve, reject) => {
      ...
      image.onerror = () => reject(
        new Error('Nie udało się wczytać grafiki odznaki.'),
      )
      image.src = src
    },
  )
...
export async function downloadAchievementCard(
  ...
  const badgeSrc = getFlightBadgeImage(
    achievement.threshold,
  )

  if (!badgeSrc) {
    throw new Error(
      'Brak grafiki tej odznaki.',
    )
  }
  ...
  if (!ctx) {
    throw new Error(
      'Przeglądarka nie obsługuje generowania karty.',
    )
  }
  ...
  ctx.fillText(
    `${completedFlights} odbytych lotów w historii`,
    540,
    948,
  )
      ...
      canvas.toBlob(
        (result) => {
          if (result) {
            ...
            reject(
              new Error('Nie udało się wygenerować karty.'),
            )
          }
        },
        mime,
