...
import { getAircraftManufacturerBadgeImage } from './achievementBadges'
...
export async function downloadAircraftManufacturerCard(
  achievement: AircraftManufacturerAchievementItem,
  nick: string,
  description: string,
  format: 'png' | 'jpg',
): Promise<void> {
  const badgeSrc = getAircraftManufacturerBadgeImage(achievement.slug)
  ...
  ctx.fillText('RODZINY I POCHODZENIE · ODZNAKA PRODUCENTA', 540, 104)
  ...
  ctx.fillText(achievement.manufacturer.toUpperCase(), 540, 820)
  ...
  ctx.fillText('Pierwszy lot tym producentem odblokował tę odznakę', 540, 962)
  ...
  downloadBlob(blob, `mapa-lotow-${safeSlug(achievement.manufacturer)}.${format}`)
}
